    ...
    if (config('asgard.iblog.config.wordpressMigrationEnable')) {
      ...
      foreach ($posts as $post) {
        ...
        if (is_null($existingPost)) {
          ...
          $newPost = app('Modules\Iblog\Repositories\PostRepository')->getItem($existingPost->post_id);
        }
        ...
        foreach ($categories as $category) {
          ...
          if (is_null($cat)) {
            ...
            $newPost->update(['category_id' => $newCategory->id]);
          } else {
            $newPost->categories()->attach($cat->id);
            $newPost->update(['category_id' => $cat->id]);
          }
        }
      }
    }
